      ...
      <ul class="list">
        <li v-for="items,indexs in list">
          <template v-if="items.type=='1'">
            ...
            <!--<span v-show="false">{{formData[items.id] = -1}}</span>-->
          ...
          <template v-if="items.type=='5'">
            <p>{{indexs+1}}.{{items.title}}</p>
            <input type="file" @change="getFile($event, indexs+1)">
          </template>
        </li>
      </ul>
        ...
        for (; i <= that.list.length; i++) {
          if (that.list[i - 1].type !== '5') {
            ...
            if (!that.fileSet[i]) {
              ...
              form.append('' + i, that.fileSet[i])
            }
          }
        }
        form.append('data', that.formData)
        if (that.flag === false) {
